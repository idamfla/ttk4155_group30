...
uint8_t can_init(uint32_t can_br, uint8_t num_tx_mb, uint8_t num_rx_mb) {
    ...
    // Disable the Parallel IO (PIO) of the Rx and Tx pins so that the peripheral controller can use
    // them
    ...
    for (int n = num_tx_mb; n <= num_rx_mb + num_tx_mb;
         n++)  // Simply one mailbox setup for all messages. You might want to apply filter for
               // them.
    {
...
uint8_t can_send(CAN_MESSAGE* can_msg, uint8_t tx_mb_id) {
    ...
    if (CAN0->CAN_MB[tx_mb_id].CAN_MSR & CAN_MSR_MRDY) {
        ...
        CAN0->CAN_MB[tx_mb_id].CAN_MDL = can_msg->data[3] << 24 | can_msg->data[2] << 16 |
                                         can_msg->data[1] << 8 | can_msg->data[0];
        CAN0->CAN_MB[tx_mb_id].CAN_MDH = can_msg->data[7] << 24 | can_msg->data[6] << 16 |
...
uint8_t can_receive(CAN_MESSAGE* can_msg, uint8_t rx_mb_id) {
    ...
    if (CAN0->CAN_MB[rx_mb_id].CAN_MSR & CAN_MSR_MRDY) {
        ...
        // Get data length
        can_msg->data_length =
            (uint8_t)((CAN0->CAN_MB[rx_mb_id].CAN_MSR & CAN_MSR_MDLC_Msk) >> CAN_MSR_MDLC_Pos);
